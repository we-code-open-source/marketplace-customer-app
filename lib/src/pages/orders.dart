...
class _OrdersWidgetState extends StateMVC<OrdersWidget> {
  ...
  _OrdersWidgetState() : super(FoodController()) {
    _con = controller;
  }
  bool loading = true;
                            ...
                            return OrderItemWidget(
                              ...
                              onAdded: (e){
                                if (_con.isSameRestaurants(
                                    _order.foodOrders[0].food)) {
                                  for (int i = 0;
                                  i < _order.foodOrders.length;
                                  i++) {
                                    _con.quantity=_order.foodOrders[i].quantity;
                                    _con.addToCart(_order.foodOrders[i].food); //<--changed to add
                                  }
                                } else {
                                  showDialog(
                                    context: context,
                                    builder: (BuildContext context) {
                                      ...
                                      return AlertDialog(
                                        ...
                                        content: Text(S.of(context).Please_choose_meals_from_the_same_restaurant),
                                        actions: [
                                          FlatButton(
