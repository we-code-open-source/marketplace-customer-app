...
class HomeController extends ControllerMVC {
  ...
  ValueNotifier<int> length = new ValueNotifier(0);
  ...
  Future<void> listenForTopRestaurants() async {
    try {
      final Stream<Restaurant> stream = deliveryAddress.value?.address == null
          ? await getNearRestaurants(
              deliveryAddress1.value, deliveryAddress1.value)
          : await getNearRestaurants(
  ...
  Future<void> listenForPopularRestaurants() async {
    try {
      ...
      stream.listen((Restaurant _restaurant) {
        ...
        loading.value = false;
        length.value = popularRestaurants.length;
      });
    } catch (e) {
      print(e.toString());
    } finally {
