...
Future<Stream<Food>> getFood(String foodId) async {
  ...
  uri = uri.replace(queryParameters: {'with': 'nutrition;restaurant;category;extras;foodReviews;foodReviews.user',
  'myLon':'${deliveryAddress.value.longitude??deliveryAddress1.value.longitude}',
  'myLat':'${deliveryAddress.value.latitude??deliveryAddress1.value.latitude}'});
  try {
    final client = new http.Client();
    final streamedRest = await client.send(http.Request('get', uri));
